use std::{ops::{Index, IndexMut, AddAssign, MulAssign, Mul, Neg}, iter, fmt::{Display, self}, cmp::max};

use ahash::AHashSet;
use galois_fields::Z64;
use num_traits::{Zero, One};
use rand::{Rng, thread_rng};

use crate::poly::UniPolynomial;

/// Univariate polynomial reconstruction using Newton interpolation
#[derive(Copy, Clone, Debug, Eq, PartialEq, Ord, PartialOrd, Hash)]
pub struct NewtonRec {
    extra_pts: usize
}

impl Default for NewtonRec {
    fn default() -> Self {
        Self { extra_pts: 1 }
    }
}

impl NewtonRec {
    pub fn new(extra_pts: usize) -> Self {
        Self { extra_pts }
    }

    pub fn rec_from_seq<I, const P: u64>(
        &self,
        pts: I
    ) -> Option<NewtonPolynomial<Z64<P>>>
    where I: IntoIterator<Item = (Z64<P>, Z64<P>)>
    {
        let mut a = TriangleMatrix::with_row_capacity(self.extra_pts + 1);
        let mut y = Vec::with_capacity(self.extra_pts + 1);
        for (pt, val) in pts.into_iter() {
            let i = a.nrows();
            y.push(pt);
            a.add_zero_row();
            a[(i, 0)] = val;
            for j in 1..=i {
                a[(i,j)] = (a[(i,j-1)] - a[(j-1,j-1)]) / (y[i] - y[j-1]);
            }
            if y.len() > self.extra_pts {
                let mut last_coeffs = (i - self.extra_pts)..=i;
                if last_coeffs.all(|d| a[(d, d)] == Zero::zero()) {
                    let coeff = Vec::from_iter((0..i - self.extra_pts).map(|c| a[(c, c)]));
                    y.truncate(max(coeff.len(), 1) - 1);
                    return Some(NewtonPolynomial {
                        coeff,
                        val: y
                    })
                }
            }
        }
        None
    }

    pub fn rec_with_ran<F, const P: u64>(
        &self,
        mut poly: F,
        mut rng: impl Rng,
    ) -> Option<NewtonPolynomial<Z64<P>>>
    where F: FnMut(Z64<P>) -> Z64<P>
    {
        let mut seen = AHashSet::new();
        self.rec_from_seq(
            iter::repeat_with(|| rng.gen())
                .filter(|pt| seen.insert(*pt))
                .map(|pt| (pt, poly(pt)))
                .take(P as usize)
        )
    }

    pub fn rec<F, const P: u64>(
        &self,
        poly: F
    ) -> Option<NewtonPolynomial<Z64<P>>>
    where F: FnMut(Z64<P>) -> Z64<P>
    {
        self.rec_with_ran(poly, thread_rng())
    }
}


#[derive(Clone, Debug, Default, Eq, PartialEq, Ord, PartialOrd, Hash)]
struct TriangleMatrix<T> {
    elem: Vec<T>,
    nrows: usize
}

impl<T> TriangleMatrix<T> {
    fn with_row_capacity(nrow: usize) -> Self {
        let capacity = row_start(nrow);
        Self {
            elem: Vec::with_capacity(capacity),
            nrows: 0
        }
    }

    fn nrows(&self) -> usize {
        self.nrows
    }
}

impl<T: Clone + Zero> TriangleMatrix<T> {
    fn add_zero_row(&mut self) {
        self.nrows += 1;
        self.elem.resize(self.elem.len() + self.nrows, T::zero())
    }
}

impl<T> Index<(usize, usize)> for TriangleMatrix<T> {
    type Output = T;

    fn index(&self, index: (usize, usize)) -> &Self::Output {
        &self.elem[row_start(index.0) + index.1]
    }
}

impl<T> IndexMut<(usize, usize)> for TriangleMatrix<T> {
    fn index_mut(&mut self, index: (usize, usize)) -> &mut T {
        &mut self.elem[row_start(index.0) + index.1]
    }
}

fn row_start(nrow: usize) -> usize {
    (nrow * (nrow + 1)) / 2
}

#[derive(Clone, Debug, Default, Eq, PartialEq, Ord, PartialOrd, Hash)]
pub struct NewtonPolynomial<T> {
    coeff: Vec<T>,
    val: Vec<T>,
}

impl<T: Zero + One + Neg<Output = T>> From<NewtonPolynomial<T>> for UniPolynomial<T>
where
    UniPolynomial<T>: AddAssign + MulAssign,
    for<'c, 'd> &'c UniPolynomial<T>: Mul<&'d T, Output = UniPolynomial<T>>,
{
    fn from(p: NewtonPolynomial<T>) -> Self {
        if p.coeff.is_empty() {
            return Self::new()
        }
        debug_assert_eq!(p.coeff.len(), p.val.len() + 1);
        let mut prod = Self::from_coeff_unchecked(vec![One::one()]);
        let mut res = UniPolynomial::new();
        for (a, y) in p.coeff.iter().zip(p.val.into_iter()) {
            res += &prod * a;
            prod *= UniPolynomial::from_coeff_unchecked(vec![-y, One::one()]);
        }
        if let Some(last) = p.coeff.last() {
            res += &prod * last;
        }
        res
    }
}

#[derive(Copy, Clone, Debug, Eq, PartialEq, Ord, PartialOrd, Hash)]
pub struct FmtNewtonPoly<'a, T: Display + One + Zero, V: Display> {
    poly: &'a NewtonPolynomial<T>,
    var: V,
}

impl<'a, T: Display + One + Zero, V: Display> FmtNewtonPoly<'a, T, V> {
    fn new(poly: &'a NewtonPolynomial<T>, var: V) -> Self {
        Self { poly, var }
    }
}

impl<T: Display + One + Zero + Eq> Display for NewtonPolynomial<T> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        FmtNewtonPoly::new(self, "x").fmt(f)
    }
}

impl<'a, T: Display + One + Zero + Eq, V: Display> Display for FmtNewtonPoly<'a, T, V> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let coeff = &self.poly.coeff;
        let val = &self.poly.val;
        let var = &self.var;
        if let Some((first, rest)) = coeff.split_first() {
            debug_assert_eq!(coeff.len(), val.len() + 1);
            write!(f, "{first}")?;
            if let Some((last_pt, rest)) = rest.split_last() {
                write!(f, " + ")?;
                let (last_coeff, val) = val.split_last().unwrap();
                for (coeff, pt) in rest.iter().zip(val.iter()) {
                    write!(f, "({var} - {pt})*({coeff} + ")?;
                }
                write!(f, "({var} - {last_pt})*{last_coeff}")?;
                for _ in 0..rest.len() {
                    write!(f, ")")?;
                }
            }
            Ok(())
        } else {
            write!(f, "0")
        }
    }
}

#[cfg(test)]
mod tests {
    use std::iter::repeat_with;

    use rand_xoshiro::rand_core::SeedableRng;

    use super::*;

    #[test]
    fn rec_poly_small() {
        const NTESTS: u32 = 10;
        const MAX_POW: u32 = 1;
        const P: u64 = 5;

        let mut rng = rand_xoshiro::Xoshiro256StarStar::seed_from_u64(1);
        let rec = NewtonRec::new(1);
        for _ in 0..NTESTS {
            let max_pow = rng.gen_range(0..=MAX_POW);
            let nterms = 2usize.pow(max_pow);
            let coeff = repeat_with(|| rng.gen::<Z64<P>>()).take(nterms).collect();
            let poly = UniPolynomial::from_coeff(coeff);
            eprintln!("{poly}");
            let reconstructed = rec.rec_with_ran(|x| poly.eval(&x), &mut rng).unwrap();
            eprintln!("{reconstructed}");
            let reconstructed: UniPolynomial<Z64<P>> = reconstructed.into();
            eprintln!("{reconstructed}");
            assert_eq!(poly, reconstructed)
        }
    }

    #[test]
    fn rec_poly_large() {
        const NTESTS: u32 = 100;
        const MAX_POW: u32 = 8;
        const P: u64 = 1152921504606846883;
        let mut rng = rand_xoshiro::Xoshiro256StarStar::seed_from_u64(1);
        let rec = NewtonRec::new(1);
        for _ in 0..NTESTS {
            let max_pow = rng.gen_range(0..=MAX_POW);
            let nterms = 2usize.pow(max_pow);
            let coeff = repeat_with(|| rng.gen::<Z64<P>>()).take(nterms).collect();
            let poly = UniPolynomial::from_coeff(coeff);
            let reconstructed = rec.rec_with_ran(|x| poly.eval(&x), &mut rng).unwrap();
            let reconstructed: UniPolynomial<Z64<P>> = reconstructed.into();
            assert_eq!(poly, reconstructed)
        }
    }
}
